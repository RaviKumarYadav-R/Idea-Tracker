      ...
      <Container className="flex justify-between items-center py-4">
        ...
        <nav>
          <ul className="flex gap-2 sm:gap-6  items-center">
            {headerSlug.map(
              (item) =>
                item.isActive && (
                  <li
                    key={item.name}
                    className="px-2 sm:px-4 py-2 bg-blue-500 text-white ttext-white rounded hover:bg-blue-600"
                  >
                    <Link
                      to={item.slug}
                      className="text-lg"
            ...
            {userLoginStatus && (
              <li>
                <Button className="cursor-pointer bg-red-600 hover:bg-red-700" onClick={handleLogout}>
